...
#   python -c "import glob,pandas as pd; pd.concat([pd.read_csv(f) for f in sorted(glob.glob('eval/outputs/nd9_capture_shards/*.csv'))]).to_csv('eval/outputs/nd9_capture.csv', index=False)"
# and send me eval/outputs/nd9_capture.csv.
...
OUT_DIR="eval/outputs/nd9_capture_shards"
mkdir -p "${OUT_DIR}"

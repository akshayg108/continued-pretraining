...
# ============================================================
# INT2 — POST-CP feature dump for Intervention-2 (spectral reversion, INT2_PREREG.md):
# bank + query per MANIFEST checkpoint (~32 ckpts x 1 dataset per task, nd12_operator.csv
# driven; needs the MAX ckpt trees on scratch).
#
# After the whole array finishes, ON THE CLUSTER first:
#   python eval/new_direction/int2_features_dump.py --verify-only
#   (full 484-cell census; freezes int2_features.sha256, or read-only-verifies
#    an existing one). Only then rsync the npz dir + sidecar back:
#   rsync -av <cluster>:.../eval/outputs/int2_features/ local:.../eval/outputs/int2_features/
#   rsync -av <cluster>:.../eval/outputs/int2_features.sha256 local:.../eval/outputs/
...
OUT_DIR="eval/outputs/int2_features"
mkdir -p "${OUT_DIR}"
...
python -u eval/new_direction/int2_features_dump.py \
    --datasets "${DATASET}" \
    --device cuda \
    --download-dir  "${DL_DIR}" \
    --processed-dir "${PROC_DIR}" \
    --outdir "eval/outputs/int2_features" \
    2>&1

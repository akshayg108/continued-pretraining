...
#SBATCH --array=0-14
#SBATCH --output=/scratch/gs4133/zhd/CP/outputs/slurm-log/nd7-placement-%A_%a.out
#SBATCH --error=/scratch/gs4133/zhd/CP/outputs/slurm-log/nd7-placement-%A_%a.err

# ============================================================
# ND7 — label-placement C(rho) + rankme on MAX checkpoints (the story-spine measurement:
# does CP move class info up the ranking?), one array task per dataset (eval/new_direction/nd7_placement.py).
...
#   python -c "import glob,pandas as pd; pd.concat([pd.read_csv(f) for f in sorted(glob.glob('eval/outputs/nd7_placement_shards/*.csv'))]).to_csv('eval/outputs/nd7_placement.csv', index=False)"
# and send me eval/outputs/nd7_placement.csv.
# Queue-friendly tip:  sbatch --array=0-14%5 run/slurm/new_direction/nd7_placement.sh
# ============================================================
...
OUT_DIR="eval/outputs/nd7_placement_shards"
mkdir -p "${OUT_DIR}"
...
python -u eval/new_direction/nd7_placement.py \
    --datasets "${DATASET}" \
    --ckpt-root "${CKPT_ROOT}" \

...
#SBATCH --array=0-14
#SBATCH --output=/scratch/gs4133/zhd/CP/outputs/slurm-log/nd10-operator-%A_%a.out
#SBATCH --error=/scratch/gs4133/zhd/CP/outputs/slurm-log/nd10-operator-%A_%a.err

# ============================================================
# ND10+ND11 — pre->post operator decomposition (dA_spec vs dA_rot) + cosine-kNN graph
# label purity/placement on MAX checkpoints, one array task per dataset (eval/new_direction/nd10_operator_transport.py). Adds one CACHED pre-CP extraction per encoder x dataset and two 2000-node Laplacian eigendecompositions per cell vs nd7 (budget: same 12h cap).
...
#   python -c "import glob,pandas as pd; pd.concat([pd.read_csv(f) for f in sorted(glob.glob('eval/outputs/nd10_operator_shards/*.csv'))]).to_csv('eval/outputs/nd10_operator.csv', index=False)"
# and send me eval/outputs/nd10_operator.csv.
# Queue-friendly tip:  sbatch --array=0-14%5 run/slurm/new_direction/nd10_operator.sh
# ============================================================
...
OUT_DIR="eval/outputs/nd10_operator_shards"
mkdir -p "${OUT_DIR}"
...
echo "=========================================="
echo "ND10 operator transport: dataset=${DATASET}"
echo "=========================================="

...
#SBATCH --time=4:00:00
#SBATCH --array=0-14
#SBATCH --output=/scratch/gs4133/zhd/CP/outputs/slurm-log/nd6-align-%A_%a.out
#SBATCH --error=/scratch/gs4133/zhd/CP/outputs/slurm-log/nd6-align-%A_%a.err

# ============================================================
# ND6 — task-model alignment C(rho) + omniscient LP risk + hubness on PRE-CP features,
# 4 encoders x 1 dataset per array task (eval/new_direction/nd6_alignment.py). ZERO ckpts,
# public timm weights only. Same protocol/scale as nd1 + one 5000-point SVD/kNN per cell.
#
# After the whole array finishes, concat shards on the login node:
#   python -c "import glob,pandas as pd; pd.concat([pd.read_csv(f) for f in sorted(glob.glob('eval/outputs/nd6_alignment_shards/*.csv'))]).to_csv('eval/outputs/nd6_alignment.csv', index=False)"
# and send me eval/outputs/nd6_alignment.csv.
# Queue-friendly tip:  sbatch --array=0-14%5 run/slurm/new_direction/nd6_alignment.sh
# ============================================================
...
OUT_DIR="eval/outputs/nd6_alignment_shards"
mkdir -p "${OUT_DIR}"
...
echo "=========================================="
echo "ND6 alignment: dataset=${DATASET}"
echo "=========================================="

python -u eval/new_direction/nd6_alignment.py \
    --datasets "${DATASET}" \
    --download-dir  "${DL_DIR}" \

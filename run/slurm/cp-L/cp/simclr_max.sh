...
#SBATCH --array=0-6
#SBATCH --output=/scratch/gs4133/zhd/CP/outputs/slurm-log/L-simclr-%A_%a.out
#SBATCH --error=/scratch/gs4133/zhd/CP/outputs/slurm-log/L-simclr-%A_%a.err

# ============================================================
# cp-L SimCLR-CP — DINOv3 ViT-L/16 scale-robustness spot check
# (eval/DESIGN_vitL_robustness.md). One array task per dataset (7), MAX only,
# 3 seeds, CP + post kNN/LP/SFT. Recipe = main grid; ViT-L adaptation only:
# a100-80g + accumulate x2 (per-step 128; ViT-L contrastive graph on 256 is memory-heavy).
# Queue tip:  sbatch --array=0-6%3 run/slurm/cp-L/cp/simclr_max.sh
# ============================================================
...
# ============================================================
# Parse optional arguments (e.g., sbatch simclr_max.sh --seed 42)
# ============================================================
OVERRIDE_SEED=""
while [[ $# -gt 0 ]]; do
    case $1 in
...
CKPT_DIR="/scratch/gs4133/zhd/CP/outputs/ckpts/cp-L/SimCLR/pretrained/${DISPLAY_NAME}/DINOv3L"
LOG_DIR="/scratch/gs4133/zhd/CP/outputs/logs/cp-L/SimCLR/pretrained/${DISPLAY_NAME}/DINOv3L"
...
run_single() {
    ...
    python -u continued_pretraining.py \
        ...
        --cache-dir ${DATA_DIR} \
        --project simclr-cp-dinov3L-${DATASET} \

...
#SBATCH --array=0-6
#SBATCH --output=/scratch/gs4133/zhd/CP/outputs/slurm-log/L-pre-%A_%a.out
#SBATCH --error=/scratch/gs4133/zhd/CP/outputs/slurm-log/L-pre-%A_%a.err

# ============================================================
# cp-L pre-CP baselines — DINOv3 ViT-L/16 scale-robustness spot check
# (eval/DESIGN_vitL_robustness.md). One array task per dataset (7 total).
# Pre kNN/LP + pre-SFT via --pre-cp-sft --no-cp (cp-siglip pre-cp idiom).
# a100-80g + accumulate x2 because the pre-SFT fine-tunes the full ViT-L.
# Queue tip:  sbatch --array=0-6%3 run/slurm/cp-L/pre-cp/dinov3L_pre.sh
# ============================================================
...
# ============================================================
# Parse optional arguments (e.g., sbatch dinov3L_pre.sh --seed 42)
# ============================================================
OVERRIDE_SEED=""
while [[ $# -gt 0 ]]; do
    case $1 in
...
# ============================================================
DATASETS=(galaxy10 dermamnist eurosat fgvc_aircraft cars196 cub200 dtd)
DISPLAYS=(Galaxy10 DermaMNIST EuroSAT FGVC_Aircraft Cars196 CUB200 DTD)
SUBPATHS=(galaxy10 med_mnist/dermamnist-size=224 eurosat fgvc_aircraft cars196 cub200 dtd)
MAXNS=(14188 7007 16200 3334 8144 5994 1880)
BLKS=(4 2 4 2 2 2 2)   # size rule: <10k -> 2 | 10-25k -> 4 (galaxy10/eurosat)
i=${SLURM_ARRAY_TASK_ID}
DATASET=${DATASETS[$i]}
DISPLAY_NAME=${DISPLAYS[$i]}
PROCESSED_SUBPATH=${SUBPATHS[$i]}
NUM_TRAINED_BLOCKS=${BLKS[$i]}
NSAMPLES=(${MAXNS[$i]})
...
CKPT_DIR="/scratch/gs4133/zhd/CP/outputs/ckpts/pre-cp-only-L/${DISPLAY_NAME}/DINOv3L"
LOG_DIR="/scratch/gs4133/zhd/CP/outputs/logs/pre-cp-only-L/${DISPLAY_NAME}/DINOv3L"
...
# ============================================================
MODEL_SIZE="ViT-L"
BACKBONE_TAG="DINOv3L"
BACKBONE_TIMM="vit_large_patch16_dinov3.lvd1689m"

EPOCHS=150
EFFECTIVE_BATCH=256
ACCUMULATE_GRAD_BATCHES=2   # ViT-L on 80g: per-step batch 128
...
# LeJEPA hyperparameters (method choice is irrelevant for --no-cp; kept for CLI parity)
LAMB=0.02
N_VIEWS=8
PROJ_DIM=128
HIDDEN_DIM=2048
...
echo ""
echo "=========================================="
echo "Starting Pre-CP eval: ${DISPLAY_NAME} (MAX only)"
...
CSV_FILE="${LOG_DIR}/${BACKBONE_TAG}_precp_results.csv"
if [ ! -f "${CSV_FILE}" ]; then

...
case "${1:-}" in
    "") ;;
    --dry-run) DRY_RUN=1 ;;
    *) echo "Usage: bash prepare.sh [--dry-run]" >&2; exit 2 ;;
esac
[ "$#" -le 1 ] || { echo "Usage: bash prepare.sh [--dry-run]" >&2; exit 2; }
...
DATASET_ID="${SLURM_ARRAY_TASK_ID:?Missing SLURM_ARRAY_TASK_ID}"
...
cd "$REPO_ROOT"
printf 'python=%s stage=prepare dataset_id=%s manifest=%s\n' "$PY" "$DATASET_ID" "$MANIFEST"
COMMAND=("$PY" -m eval.heldout_cp prepare --manifest "$MANIFEST"
    --dataset-id "$DATASET_ID" --cache-dir "$CACHE_DIR" --num-workers "$WORKERS")

#!/bin/bash
#SBATCH --job-name=expJ-transport
...
#SBATCH --array=0-14
#SBATCH --output=/scratch/gs4133/zhd/CP/outputs/slurm-log/expJ-transport-%A_%a.out
...
#   python -c "import glob,pandas as pd; pd.concat([pd.read_csv(f) for f in sorted(glob.glob('eval/outputs/transport_field_shards/*.csv'))]).to_csv('eval/outputs/transport_field_max.csv', index=False)"
# then run the scorer (CPU):  python eval/adjudicate/transport_law.py
# and send me eval/outputs/transport_field_max.csv (+ transport_stats.csv if you ran it).
# Queue-friendly tip:  sbatch --array=0-14%5 run/slurm/eval/exp_j_transport.sh
# ============================================================
...
OUT_DIR="eval/outputs/transport_field_shards"
mkdir -p "${OUT_DIR}"
...
# ============================================================
# Run: all 4 methods x 3 encoders x this dataset @ MAX
# ============================================================
echo "=========================================="

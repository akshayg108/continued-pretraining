...
#SBATCH --time=6:00:00
#SBATCH --array=0-14
#SBATCH --output=/scratch/gs4133/zhd/CP/outputs/slurm-log/expE-geoclass-%A_%a.out
...
# Feeds P-A (cdnv/wb_ratio), P-B (task_energy), P-C1 (rankme/alpha/ID).
#
# After the whole array finishes, concat shards on the login node:
#   python -c "import glob,pandas as pd; pd.concat([pd.read_csv(f) for f in sorted(glob.glob('eval/outputs/geometry_class_shards/*.csv'))]).to_csv('eval/outputs/geometry_class_15.csv', index=False)"
# and send me eval/outputs/geometry_class_15.csv.
# Queue-friendly tip: submit with a throttle ->  sbatch --array=0-14%5 run/slurm/eval/exp_e_geometry_class.sh
# ============================================================
...
OUT_DIR="eval/outputs/geometry_class_shards"
mkdir -p "${OUT_DIR}"
...
# ============================================================
# Run: all 4 encoders x this dataset (+ ImageNet per encoder)
# ============================================================
echo "=========================================="

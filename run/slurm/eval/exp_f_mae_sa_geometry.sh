...
#SBATCH --array=0-14
#SBATCH --output=/scratch/gs4133/zhd/CP/outputs/slurm-log/expF-maesa-%A_%a.out
...
# and ImageNet-val through that head, and recomputes the F1 geometry metrics.
# Decides P-C2: is the MAE F1 inversion genuine geometry or a mean-pool artifact?
#
# After the whole array finishes, concat shards on the login node:
#   python -c "import glob,pandas as pd; pd.concat([pd.read_csv(f) for f in sorted(glob.glob('eval/outputs/geometry_mae_sa_shards/*.csv'))]).to_csv('eval/outputs/geometry_mae_sa.csv', index=False)"
# and send me eval/outputs/geometry_mae_sa.csv.
# Queue-friendly tip:  sbatch --array=0-14%5 run/slurm/eval/exp_f_mae_sa_geometry.sh
# ============================================================
...
OUT_DIR="eval/outputs/geometry_mae_sa_shards"
mkdir -p "${OUT_DIR}"

# ============================================================
# Stage this dataset's processed cache + ImageNet-val node-local
# ============================================================
SRC_PROC="${PROC_DIR}/${PROCESSED_SUBPATH}"
if [ -d "${SRC_PROC}" ]; then
    NEED_KB=$(( $(du -sk "${SRC_PROC}" | awk '{print $1}') + $(du -sk "${IMAGENET_DIR}" 2>/dev/null | awk '{print $1}') + 5*1024*1024 ))
    STAGE_ROOT=""
...
# ============================================================
# Run: MAE encoder + SA readout, this dataset only
# ============================================================
echo "=========================================="

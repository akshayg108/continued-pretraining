#!/bin/bash
#SBATCH --job-name=pre-food-clip-max
...
#SBATCH --output=/scratch/gs4133/zhd/CP-MAE/outputs/slurm-log/pre-cp-food101-clip-max-%j.out
#SBATCH --error=/scratch/gs4133/zhd/CP-MAE/outputs/slurm-log/pre-cp-food101-clip-max-%j.err
...
CKPT_DIR="/scratch/gs4133/zhd/CP-MAE/outputs/ckpts/pre-cp-only/Food101/CLIP/max"
LOG_DIR="/scratch/gs4133/zhd/CP-MAE/outputs/logs/pre-cp-only/Food101/CLIP/max"
...
if len(knn_f1s) > 0 or len(linear_f1s) > 0 or len(sft_f1s) > 0:
    def cls_std(vals):
        if len(vals) == 0:
            return "", ""
        m = statistics.cls(vals)
        s = statistics.stdev(vals) if len(vals) > 1 else 0.0
        return f"{m:.6f}", f"{s:.6f}"

    knn_cls, knn_std = cls_std(knn_f1s)
    lin_cls, lin_std = cls_std(linear_f1s)
    sft_cls, sft_std = cls_std(sft_f1s)

    with open(csv_file, "a") as f:
        f.write(f"{backbone_tag},{display_name},{n_samples},{model_size},average,{knn_cls},{knn_std},{lin_cls},{lin_std},{sft_cls},{sft_std}\n")

    print(f"  [{backbone_tag}] {display_name} n={n_samples}: knn_f1={knn_cls}+-{knn_std} linear_f1={lin_cls}+-{lin_std} sft_f1={sft_cls}+-{sft_std}")
else:
...
echo ""
echo "=========================================="
echo "Starting Pre-CP-Only: ${DISPLAY_NAME} (${BACKBONE_TAG}, max)"

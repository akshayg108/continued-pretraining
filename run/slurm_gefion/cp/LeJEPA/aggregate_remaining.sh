#!/bin/bash
#SBATCH --job-name=lejepa-rem-agg
...
#SBATCH --output=/dcai/projects/iu_0092/projects/cp/outputs/slurm-logs/lejepa-remaining-agg-%j.out
...
STATUS_CSV="${STATUS_CSV:-${COMBINED_DIR}/lejepa_remaining_status_${ARRAY_JOB_ID}.csv}"
COMBINED_CSV="${COMBINED_CSV:-${COMBINED_DIR}/lejepa_remaining_combined_results_${ARRAY_JOB_ID}.csv}"
...
def log_dir_for(row):
    if row["group"] == "random":
        rel = Path("cp/LeJEPA/random") / row["display_name"]
    else:
        rel = Path("cp/LeJEPA/pretrained") / row["display_name"] / row["backbone_tag"]
...
def csv_path_for(row):
    return log_dir_for(row) / f"{row['backbone_tag']}_lejepa_cp_remaining_results.csv"
...
echo ""
echo "=========================================="
echo "Remaining aggregation completed"
echo "End Time: $(date)"
echo "=========================================="

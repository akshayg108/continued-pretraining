#!/bin/bash
#SBATCH --job-name=simclr-max-agg
...
#SBATCH --output=/dcai/projects/iu_0092/projects/cp/outputs/slurm-logs/simclr-max-agg-%j.out
...
OUTPUT_ROOT="/dcai/projects/iu_0092/projects/cp/outputs/logs/cp/SimCLR"
...
STATUS_CSV="${STATUS_CSV:-${COMBINED_DIR}/simclr_max_status_${ARRAY_JOB_ID}.csv}"
COMBINED_CSV="${COMBINED_CSV:-${COMBINED_DIR}/simclr_max_combined_results_${ARRAY_JOB_ID}.csv}"
...
def log_dir_for(row):
    rel = Path("cp/SimCLR/pretrained") / row["display_name"] / row["backbone_tag"] / "all"
...
def csv_path_for(row):
    return log_dir_for(row) / f"{row['backbone_tag']}_simclr_cp_results.csv"
...
echo ""
echo "=========================================="
echo "SimCLR MAX aggregation completed"
echo "End Time: $(date)"
echo "=========================================="

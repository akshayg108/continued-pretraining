#!/bin/bash
#SBATCH --job-name=mae-max-eval-agg
...
#SBATCH --output=/dcai/projects/iu_0092/projects/cp/outputs/slurm-logs/mae-max-eval-agg-%j.out
...
JOB_FILE=""
EVAL_POOL_STRATEGY="cls"

while [[ $# -gt 0 ]]; do
    case "$1" in
        --manifest) MANIFEST="$2"; shift 2 ;;
        --job-file) JOB_FILE="$2"; shift 2 ;;
        --eval-pool-strategy) EVAL_POOL_STRATEGY="$2"; shift 2 ;;
...
echo "Eval pool strategy: ${EVAL_POOL_STRATEGY}"
echo "=========================================="
...
STATUS_CSV="${STATUS_CSV:-${COMBINED_DIR}/mae_max_eval_fix_status_${ARRAY_JOB_ID}.csv}"
COMBINED_CSV="${COMBINED_CSV:-${COMBINED_DIR}/mae_max_eval_fix_combined_results_${ARRAY_JOB_ID}.csv}"

echo "Status CSV: ${STATUS_CSV}"
echo "Combined CSV: ${COMBINED_CSV}"

"${PYTHON_BIN}" - "${MANIFEST}" "${STATUS_CSV}" "${COMBINED_CSV}" "${EVAL_POOL_STRATEGY}" <<'PY'
import csv
import json
import statistics
import sys
...
def rel_dir_for(row, leaf):
    return Path("cp/MAE/pretrained") / row["display_name"] / row["backbone_tag"] / leaf


def source_leaf_for(row):
    if eval_pool_strategy == "all" or row["pool_strategy"] == eval_pool_strategy:
        return "all_eval_fix"
    return "all"


def log_dir_for(row):
    return output_root / rel_dir_for(row, source_leaf_for(row))
...
def csv_path_for(row):
    return output_root / rel_dir_for(row, "all_eval_fix") / f"{row['backbone_tag']}_mae_cp_results.csv"
...
for row in manifest_rows:
    ...
    missing = []
    per_seed_rows = []
    result_source = source_leaf_for(row)
...
echo ""
echo "=========================================="
echo "MAE MAX eval-fix aggregation completed"
echo "End Time: $(date)"
echo "=========================================="

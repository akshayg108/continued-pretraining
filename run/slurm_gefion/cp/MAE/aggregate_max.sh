...
#SBATCH --output=/dcai/projects/iu_0092/projects/cp/outputs/slurm-logs/mae-max-agg-%j.out
...
STATUS_CSV="${STATUS_CSV:-${COMBINED_DIR}/mae_max_status_${ARRAY_JOB_ID}.csv}"
COMBINED_CSV="${COMBINED_CSV:-${COMBINED_DIR}/mae_max_combined_results_${ARRAY_JOB_ID}.csv}"
...
def log_dir_for(row):
    rel = Path("cp/MAE/pretrained") / row["display_name"] / row["backbone_tag"] / "all"
...
def csv_path_for(row):
    return log_dir_for(row) / f"{row['backbone_tag']}_mae_cp_results.csv"
...
echo ""
echo "=========================================="
echo "MAE MAX aggregation completed"
echo "End Time: $(date)"
echo "=========================================="

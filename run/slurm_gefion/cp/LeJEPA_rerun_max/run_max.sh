#!/bin/bash
#SBATCH --job-name=lejepa-rerun-max
...
#SBATCH --array=0-0
#SBATCH --output=/dcai/projects/iu_0092/projects/cp/outputs/slurm-logs/lejepa-rerun-max-%A_%a.out
...
rel_dir = f"cp/LeJEPA_rerun_max/pretrained/{row['display_name']}/{row['backbone_tag']}/all"
...
echo "=========================================="
echo "Starting LeJEPA-CP rerun MAX run"
echo "Run ID: ${RUN_ID}"
echo "Dataset: ${DISPLAY_NAME} (${DATASET})"
echo "Backbone: ${BACKBONE_TAG} (${BACKBONE_TIMM})"
echo "n_samples=${N_SAMPLES} seed=${SEED}"
echo "num_trained_blocks=${NUM_TRAINED_BLOCKS} effective_batch=${EFFECTIVE_BATCH} batch_size=${BATCH_SIZE} accumulate_grad_batches=${ACCUMULATE_GRAD_BATCHES}"
echo "pool_strategy=${POOL_STRATEGY} lamb=${LAMB} n_views=${N_VIEWS}"
...
echo ""
echo "=========================================="
echo "LeJEPA-CP rerun MAX run completed"
echo "Run ID: ${RUN_ID}"
echo "End Time: $(date)"
echo "=========================================="

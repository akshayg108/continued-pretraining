...
class ABMILPHead(nn.Module):
    def __init__(
        ...
        if self.self_attention_apply_to != "none":
            # Lazy import — Attention only needed when self_attention is enabled.
            # In our depth=1 minimal config, self_attention="none", so this path is skipped.
            from .attention import Attention  # noqa: F401
            self.self_attn = Attention(dim, num_heads=1)
        else:

...
def main():
    ...
    print(
        f"DIET CP: {args.dataset} | {args.backbone} | freeze={freeze_epochs} warmup={warmup_epochs}"
    )
    ...
    ckpt_path = str(
        checkpoint_dir
        / f"diet_cp_{args.dataset}_{args.backbone.replace('/', '_')}.ckpt"
    )
    run_training(
        module, data, args, ds_cfg, embed_dim, freeze_epochs, logger, ckpt_path, method="diet"
    )
    run_final_eval(

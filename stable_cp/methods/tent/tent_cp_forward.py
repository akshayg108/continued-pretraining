...
def configure_model_for_tent(
    backbone, classifier, tent_mode="norm_only", num_trained_blocks=-1
):
    ...
    if tent_mode == "norm_only":
        ...
        if num_trained_blocks > 0:
            ...
            if layers is not None:
                ...
                print(
                    f"TENT mode: combined - Training blocks {start_idx}-{total_blocks - 1} + all norm layers + classifier"
                )
            else:
                print(
                    f"TENT mode: combined - Could not find transformer layers, only norm layers + classifier enabled"
                )
        else:
            print(
                f"TENT mode: combined - Only norm layers + classifier enabled (num_trained_blocks={num_trained_blocks})"
            )

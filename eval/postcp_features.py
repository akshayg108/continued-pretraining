...
_PREFIX_CANDIDATES = [
    "", "backbone.", "model.backbone.", "model.", "encoder.", "net.backbone.",
    "student.backbone.", "student.", "teacher.backbone.", "module.backbone.", "module.",
]


def load_cp_backbone(ckpt_path, timm_id, device):
    """Build the timm arch and load the post-CP backbone weights (prefix auto-detected)."""
    model = timm.create_model(timm_id, pretrained=False, num_classes=0)
    target = set(model.state_dict().keys())
    ckpt = torch.load(ckpt_path, map_location="cpu", weights_only=False)
    sd = ckpt.get("state_dict", ckpt) if isinstance(ckpt, dict) else ckpt

    best_pre, best_hits, best_sd = None, -1, None
    for pre in _PREFIX_CANDIDATES:
        stripped = {k[len(pre):]: v for k, v in sd.items() if k.startswith(pre)}
        hits = len(target & set(stripped.keys()))
        if hits > best_hits:
            best_pre, best_hits, best_sd = pre, hits, stripped
    print(f"  backbone prefix='{best_pre}'  matched {best_hits}/{len(target)} timm keys")
    ...
    if best_hits < 0.5 * len(target):
        print("  WARNING: <50% keys matched — verify --ckpt / prefix. First 10 checkpoint keys:")
